...
from rich.syntax import Syntax, RICH_SYNTAX_THEMES
...
from rugosa.emulation.objects import File, Object
...
fg_colors = [c.value for c in cmd2.Color]
syntax_themes = [*RICH_SYNTAX_THEMES.keys(), *pygments_styles.get_all_styles()]
...
class App(cmd2.Cmd):
    ...
    def __init__(self, dis: Disassembler, logo=True, history_file: str = None, startup_script: str = None, precache=True):
        ...
        self.prompt_color = cmd2.Color.GREEN.value
        self.add_settable(cmd2.Settable(
            "prompt_color", str,
            "Color of prompt. ('default' disables color)", self,
            choices=fg_colors
        ))
        self.syntax_theme = "ansi_dark"
        self.add_settable(cmd2.Settable(
            "syntax_theme", str,
            "Source code syntax theme.", self,
            choices=syntax_themes
        ))
        self.add_settable(cmd2.Settable(
            "max_undo_items", int,
        ...
        self._ctx_gen = None
        # We can't allow running secondary threads if in IDA or using idalib.
        self._allow_threads = not (dis.name.lower() == "ida" and dis.is_local)
        self._precache = precache and self._allow_threads
        self._function_names = []
        self._import_names = []
        self._threads = []
        self._reset(init=True)

        self.set_window_title(f"Rugosa - {dis.input_path}")
        if logo:
            self.intro = (
    ...
    def _run_python(self, *, pyscript: Optional[str] = None) -> Optional[bool]:
        self._update_py_locals()
        return super()._run_python(pyscript=pyscript)

    def do_ipy(self, *args, **kwargs):
        """
        ...
        """
        self._update_py_locals()
        return super().do_ipy(*args, **kwargs)
    ...
    def _set_prompt(self):
        ...
        prompt += ")> "
        self.prompt = cmd2.stylize(prompt, style=Style(color=self.prompt_color))
    ...
    def _obtain_address(self, target: str, default=MISSING) -> Optional[int | Any]:
        """
    ...
    def ptable(self, tabular_data, headers="keys"):
        ...
        self.ppaged(tabulate(tabular_data, headers=headers, tablefmt=self.tablefmt))

    def _sanitize_enums(self, value):
        """
        Convert enums to their names.
        Have to do this here, because IntEnum/IntFlag's ignore the custom encoder.
        """
        if isinstance(value, (list, tuple)):
            return value.__class__(map(self._sanitize_enums, value))
        elif isinstance(value, dict):
            return value.__class__(map(self._sanitize_enums, value.items()))
        elif isinstance(value, Enum):
            return value.name
        else:
            return value

    def pjson(self, obj, **kwargs):
        """Prints object as json."""
        kwargs["indent"] = kwargs.get("indent", 2)
        kwargs["cls"] = kwargs.get("cls", CustomEncoder)
        self.ppaged(json.dumps(self._sanitize_enums(obj), **kwargs))
    ...
    def _get_status(self) -> str:
        return tabulate([
            ...
            ["Disassembler", f"{self.dis.name} v{self.dis.version}"],
    ...
    def do_alternative(self, args):
        ...
        for line in difflib.Differ().compare(old, new):
            ...
            self.poutput(cmd2.stylize(line, style=Style(color=color)))
    ...
    def do_disassembly(self, args):
        ...
        self.poutput(cmd2.stylize(f" -> 0x{insn.address:08x}: {insn.text}", Style(color=cmd2.Color.LIGHT_CYAN1)))
    ...
    def do_code(self, args):
        ...
        if func := self.dis.get_function(address, None):
            if source_code := func.source_code:
                if args.syntax_highlight:
                    source_code = Syntax(source_code, "c", theme=self.syntax_theme)
                self.poutput(source_code)
            else:
                self.perror(f"Source code not available at 0x{address:08x}")
        else:
    ...
    strings_parser.add_argument(
        "--static",
        action="store_true",
        help="Extract static strings",
    )
    strings_parser.add_argument(
        "--stack",
        action="store_true",
        help="Extract dynamically allocated strings from stack",
    )
    strings_parser.add_argument(
        "--min",
        type=int,
    ...
    strings_parser.add_argument(
        "--simple",
        action="store_true",
        help="Only display the string value as a raw list."
    )
    strings_parser.add_argument(
        "--raw",
        action="store_true",
    ...
    @functools.cache
    def _find_stack_strings(self, min_length):
        """Cached extraction of stack strings"""
        return list(rugosa.find_stack_strings(self.dis, min_length=min_length))

    @functools.cache
    def _find_static_strings(self, min_length):
        """Cached extract of static strings"""
        return list(self.dis.strings(min_length=min_length))
    ...
    def do_strings(self, args):
        ...
        If --static or --stack is used, only strings of that type is extracted.
        """
        if not (args.static or args.stack):
            args.static = True
            args.stack = True
        ...
        if args.target:
            ...
            if args.static:
                for string in self._find_static_strings(args.min):
                    if string.address == address:
                        if args.raw:
                            sys.stdout.buffer.write(bytes(string))
                        else:
                            self.poutput(str(string))
                        return

            if args.stack:
                for string in self._find_stack_strings(args.min):
                    if string.dec_source == address:
                        if args.raw:
                            sys.stdout.buffer.write(bytes(string))
                        else:
                            self.poutput(str(string))
                ...
                table_data.extend([
                    {
                        ...
                        "variable": self.dis.get_name(string.address),
                        "size": len(bytes(string)),
                        "type": "STATIC",
                        "string": str(string),
                    }
                    for string in self._find_static_strings(args.min)
            ...
            if args.stack:
                table_data.extend([
                    {
                        "address": f"0x{string.dec_source:08x}",
                        "variable": string.enc_source.name,
                        "size": len(bytes(string)),
                        "type": "STACK",
                        "string": str(string),
                    }
    ...
    def do_search(self, args):
        ...
        tabular_data = []
        for i, match in enumerate(pattern.finditer(self.dis, args.segment), start=1):  # noqa
    ...
    def pactions(self, actions, json=False):
        """Prints actions"""
        if json:
            self.pjson([
                {"action": action.__class__.__name__, **dict(action)}
            ...
            self.ptable([
                {
                    ...
                    "handle": hex(action.handle) if getattr(action, "handle", None) else None,
                    "attributes": ", ".join(
                        f"{name}={trepr(value)}" for name, value in action if name not in ("ip", "handle")),
                }
                for action in actions
            ])

    actions_parser = cmd2.Cmd2ArgumentParser()
    ...
    @cmd2.with_category(CATEGORY_EMULATION_ARTIFACTS)
    @cmd2.with_argparser(actions_parser)
    def do_actions(self, args):
        """
        Prints out the actions that have taken place within the current context.
        """
        self.pactions(self.ctx.actions, json=args.json)

    def pobjects(self, objects: Iterable[Object], json=False):
        """Prints objects"""
        if json:
            self.pjson([obj.as_dict() for obj in objects])
        else:
            tabular_data = []
            for object in objects:
                entry = object.as_dict()
                if entry["type"] == "File":
                    entry["size"] = len(entry["data"])
                references = entry.pop("references")
                tabular_data.append({
                    "address": hex(references[0]) if references else None,
                    "type": entry.pop("type"),
                    "handle": hex(entry.pop("handle")),
                    "attributes": ", ".join(f"{name}={trepr(value)}" for name, value in entry.items())
                })
            self.ptable(tabular_data)

    objects_parser = cmd2.Cmd2ArgumentParser()
    ...
    def do_objects(self, args):
        ...
        """
        self.pobjects(self.ctx.objects, json=args.json)

    def pfiles(self, files, json=False):
        """Prints files."""
        if json:
            self.pjson([file.as_dict() for file in files])
        else:
            tabular_data = []
            for file in files:
                data = file.data  # cache to avoid repetitive calls.
                tabular_data.append({
                    "address": hex(file.references[0]),
                    ...
                    "md5": file.md5 if data else None,
                    "data": trepr(data) if data else None,
                })
            self.ptable(tabular_data)

    def export_files(self, files: Iterable[File], output_dir: Path):
        """Saves file data to given output directory."""
        ...
        for file in files:
            if file.data:
                ...
                filename = address
                if file.path:
                    filename += f"_{PureWindowsPath(file.path).name}"
                else:
                    filename += f"_{hashlib.md5(file.data).hexdigest()}"
                path = output_dir / filename
                count = 0
                while path.exists():
                    count += 1
                    path = path.with_name(filename + f"_{count}")
                path.write_bytes(file.data)
                self.pfeedback(f"[+] Wrote {len(file.data)} byte file from {address} to {path}")
    ...
    files_parser.add_argument(
        "--export",
        type=Path,
        help="Directory path to save the files with data."
    )
    ...
    def do_files(self, args):
        ...
        if args.handle:
            ...
            self.pfiles(self.ctx.files, json=args.json)

        if export_dir := args.export:
            self.export_files(self.ctx.files, export_dir)

    def pregkeys(self, regkeys, json=False):
        """Prints registry keys"""
        if json:
            self.pjson([regkey.as_dict() for regkey in regkeys])
        else:
            self.ptable([
                {
                    "address": hex(regkey.references[0]),
    ...
    regkeys_parser = cmd2.Cmd2ArgumentParser()
    regkeys_parser.add_argument(
        "--json",
        action="store_true",
        help="Print results as json."
    )
    ...
    @cmd2.with_category(CATEGORY_EMULATION_ARTIFACTS)
    @cmd2.with_argparser(regkeys_parser)
    def do_regkeys(self, args):
        """
        Prints out the registry key objects that have been generated within the current context.
        """
        self.pregkeys(self.ctx.reg_keys, json=args.json)

    def pservices(self, services, json=False):
        """Print the services"""
        if json:
            self.pjson([service.as_dict() for service in services])
        else:
            self.ptable([
                {
                    "address": hex(service.references[0]),
    ...
    services_parser = cmd2.Cmd2ArgumentParser()
    ...
    def do_find(self, args):
        """Finds and extracts high level objects or actions."""
        config = {
            "start": self._obtain_address(args.start) if args.start else None,
            "depth": args.depth,
            "call_depth": args.call_depth,
            "follow_loops": args.follow_loops,
            "init_context": self.ctx if args.keep else None,
        ...
        if args.target == "actions":
            self.pactions(self.emu.find_actions(**config), json=args.json)
        elif args.target == "objects":
            objects = list(self.emu.find_objects(**config))
            self.pobjects(objects, json=args.json)
            if export_dir := args.export:
                self.export_files([obj for obj in objects if isinstance(obj, File)], export_dir)
        elif args.target == "files":
            files = list(self.emu.find_files(**config))
            self.pfiles(files, json=args.json)
            if export_dir := args.export:
                self.export_files(files, export_dir)
        elif args.target == "regkeys":
            self.pregkeys(self.emu.find_reg_keys(**config), json=args.json)
        elif args.target == "services":
            self.pservices(self.emu.find_services(**config), json=args.json)
...
def temporary_file(binary_path: Path) -> Iterable[Path]:
...
def main():
    ...
    parser.add_argument(
        "-b", "--backend",
        choices=[dragodis.BACKEND_GHIDRA.lower(), dragodis.BACKEND_IDA.lower(), dragodis.BACKEND_VIVISECT.lower()],
    ...
    parser.add_argument(
        "-s", "--startup-script",
        default=settings.startup_script,
        help="Path to startup script to be executed. Defaults to 'startup_script' setting."
    )
    parser.add_argument(
        "--nocache",
        action="store_true",
