...
import sys
from typing import Iterable, Tuple, Union, Iterator, TYPE_CHECKING, Unpack

import dragodis
from dragodis.interface import GlobalVariable
...
def find_api_resolve_strings(dis: dragodis.Disassembler) -> Iterable[Tuple[int, str]]:
    ...
    """
    seen = set()
    from rugosa.emulation import Emulator  # avoid circular import
...
class DecodedString:
    ...
    def __repr__(self) -> str:
        ...
        if isinstance(dec_source, int):
            dec_source = hex(dec_source)

        enc_source = self.enc_source
        if isinstance(enc_source, int):
            enc_source = hex(enc_source)

        if enc_source and dec_source:
            return f"<DecodedString {enc_source} @ {dec_source} = '{self}' ({self.encoding})>"
        elif enc_source or dec_source:
            return f"<DecodedString {enc_source or dec_source} = '{self}' ({self.encoding})>"
        else:
            return f"<DecodedString '{self}' ({self.encoding})>"
    ...
    def patch(self, dis: dragodis.Disassembler, fill_char=b"\x00", rename=True):
        ...
        if self.dec_source:
            ...
            elif isinstance(dec_source, GlobalVariable):
...
def find_stack_strings(
    ...
    :param annotate: Whether to add a comment at the location of the found stack string.
    ...
    # Disable all instructions that aren't necessary for correct results
    emulator.disable_all()
    emulator.enable(
        "push", "pop", "call",
        "add", "sub", "xor", "and", "dec", "inc",
        "mov", "lea",
    )
    for string in emulator.find_stack_strings(start=start, min_length=min_length, **config):
        if annotate:
            dis.get_line(string.dec_source).set_comment(f"Stack String: '{string.display_name}'")
        yield string
