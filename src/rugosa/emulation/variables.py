...
class Variable:
    ...
    :var references: List of instruction pointers where the variables were encountered.
    """
    ...
    @property
    def static(self) -> dragodis.interface.Variable:
        """The underlying static disassembler variable."""
    ...
    def data_type_size(self) -> int:
        ...
        # Pulling base size to account for static array types.
        return self._variable.data_type.base.size

    @property
    def count(self) -> int:
        """Count of elements in the array."""
        count = self._variable.data_type.count
        if count > 1:  # static array
            return count
        else:
