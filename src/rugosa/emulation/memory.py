...
import io
import os
from typing import TYPE_CHECKING, Tuple, List, Optional, Iterable, Union, TypedDict, Unpack
import warnings
...
class MemoryArgs(TypedDict, total=False):
    """
    Sets the maximum amount of memory allowed to read/write and fallback sizes
    for when the limit is hit.
    """
    max_read_size: int
    max_write_size: int
...
class Memory:
    ...
    def __init__(self, cpu_context: ProcessorContext, _cache=None, **kwargs: Unpack[MemoryArgs]):
        """Initializes Memory object."""
        self.max_read_size = kwargs.get("max_read_size", settings.memory.max_read_size)
        self.max_write_size = kwargs.get("max_write_size", settings.memory.max_write_size)
        self.fallback_read_size = kwargs.get("fallback_read_size", settings.memory.fallback_read_size)
        self.fallback_write_size = kwargs.get("fallback_write_size", settings.memory.fallback_write_size)
    ...
    def __deepcopy__(self, memo):
        ...
        copy.max_read_size = self.max_read_size
        copy.max_write_size = self.max_write_size
        copy.fallback_read_size = self.fallback_read_size
        copy.fallback_write_size = self.fallback_write_size
    ...
    def read(self, address: int, size: int) -> bytes:
        ...
        if size > self.max_read_size:
            logger.error(
                ...
                "Ignoring request due to being larger than %d bytes. (Reading the first %d bytes instead.)",
                size, address, self.max_read_size, self.fallback_read_size
            )
            size = self.fallback_read_size
    ...
    def write(self, address: int, data: bytes) -> int:
        ...
        if size > self.max_write_size:
            logger.error(
                ...
                "Ignoring request due to being larger than %d bytes. (Writing the first %d bytes instead.)",
                size, address, self.max_write_size, self.fallback_write_size
            )
            data = data[:self.fallback_write_size]
        size = len(data)
    ...
    def copy(self, src: int, dst: int, size: int):
        ...
        """
        max_size = min(self.max_read_size, self.max_write_size)
        fallback_size = min(self.fallback_read_size, self.fallback_write_size)
        if size > max_size:
            logger.error(
                "Attempted to copy %d bytes from 0x%08X. "
                "Ignoring request due to being larger than %d bytes. (Copying the first %d bytes instead.)",
                size, src, max_size, fallback_size
            )
            size = fallback_size
