...
class Operand:
    ...
    @value.setter
    def value(self, value: Union[int, float, bytes]):
        """
        ...
        if isinstance(value, int):
            ...
            # Mask off to avoid overflow.
            masked_value = value & utils.get_mask(self.width)
            if value != masked_value:
                logger.warning(
                    f"0x%X:%d operand value 0x%X was masked to 0x%X to avoid overflow error.",
                    self.ip, self.idx, value, masked_value
                )
                value = masked_value
