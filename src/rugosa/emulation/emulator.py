r"""
Main interface for emulating functions.

e.g.
    with dragodis.open_program(r"C:\input.exe") as dis:
        emulator = rugosa.Emulator(dis)
        ctx = emulator.context_at(0x123456)
"""
from __future__ import annotations

import collections
import contextlib
import logging
from copy import deepcopy
from typing import Optional, Tuple, List, Iterable, Callable, Union, TypedDict, Unpack

import dragodis
from dragodis.interface import BasicBlock

from rugosa.emulation.functions import FunctionArgument
from rugosa.emulation.instruction import Instruction
from rugosa.emulation.monitor import Monitor, CallbacksMonitor
from rugosa.emulation.monitors import *
from rugosa.emulation.tracing import TracingMixin
from rugosa.config import settings

from . import call_hooks
from .x86_64 import x86_64ProcessorContext
from .ARM import ARMProcessorContext
from .cpu_context import ProcessorContext
from .flowchart import iter_paths

logger = logging.getLogger(__name__)


class IterContextArgs(TypedDict, total=False):
    depth: int
    call_depth: int
    exhaustive: bool
    follow_loops: bool
    init_context: ProcessorContext


class IterExhaustArgs(IterContextArgs, total=False):
    start: int
    ignore_libraries: bool
    scope: str
    pre: bool


class MemoryArgs(TypedDict, total=False):
    max_read_size: int
    max_write_size: int
    fallback_read_size: int
    fallback_write_size: int
    stack_base: int
    stack_offset: int
    base_offset: int


class EmulatorArgs(TypedDict, total=False):
    """
    :param max_instructions:
        Maximum number of instructions to allow when emulating loop following code.
        This is used since it is possible that the end instruction would never get reached.
    :param branch_tracking:
        When forcing emulation to go down the incorrect branch in order to reach the desired
        end address, branch_tracking is used to try to tweak the registers to make the branching
        condition true.
        This can be helpful to ensure the rest of the emulation is done correctly, however
        this will cause emulation to run slower.
        So this option allows you to turn it off when the feature is not necessary.
    :param variable_tracking:
        During emulation, high level variables defined by the underlying disassembler are recorded.
        This can be turned off to improve performance if the feature isn't necessary.
    :param teleport:
        Whether to allow the emulator instance to be teleported into the underlying disassembler.
        This is done for performance reasons, however in some cases if you are doing some deeper customizations
        to the emulator you might run into problems. Therefore, we have the option to turn it off.
    :param memory:
        Overrides `memory` settings.
    """
    max_instructions : int
    branch_tracking: bool
    variable_tracking: bool
    teleport: bool
    memory: MemoryArgs


class Emulator(TracingMixin):
    """
    This class is the controller for emulating and getting contexts.
    This is used for:
        - Initial interfacing with emulation.
        - Holding the Dragodis disassembler.
        - Managing the hooks used for function calls.
        - Store global emulation options.
    """

    def __new__(cls, disassembler: dragodis.Disassembler, *args, **kwargs: Unpack[EmulatorArgs]) -> Emulator:
        teleport = kwargs.get("teleport", settings.emulation.teleport)
        # If we are using IDA, teleport the emulator instance into the IDA interpreter.
        # This helps to greatly improve performance!
        if teleport and disassembler.name.lower() == "ida" and not disassembler.is_local:
            @disassembler.teleport
            def _Emulator(dis, *args, **kwargs):
                import rugosa
                emulator = rugosa.Emulator(dis, *args, **kwargs)
                emulator.teleported = True
                return emulator
            return _Emulator(disassembler, *args, **kwargs)
        else:
            return super().__new__(cls)

    def __init__(self, disassembler: dragodis.Disassembler, **kwargs: Unpack[EmulatorArgs]):
        """
        Initialize an Emulator instance

        :param disassembler: Backend dragodis disassembler used for obtaining instructions.
        :param kwargs: Extra arguments to configure Emulator. (see EmulatorArgs)
        """
        super().__init__()
        self.disassembler = disassembler
        # Determine the appropriate class to use for generated contexts based on arch name.
        self.arch = disassembler.processor_name
        if disassembler.is_x86:
            self._context_class = x86_64ProcessorContext
        elif disassembler.is_arm:
            self._context_class = ARMProcessorContext
        else:
            raise NotImplementedError(f"Architecture not supported: {self.arch}")

        self._flowchart_cache = {}
        self._memory_cache = {}
        self._call_hooks: dict[str|int, Callable] = call_hooks.BUILTINS.copy()
        self._opcode_hooks: dict[str, Callable] = self._context_class.OPCODES.copy()
        self.max_instructions = kwargs.get("max_instructions", settings.emulation.max_instructions)
        self.branch_tracking = kwargs.get("branch_tracking", settings.emulation.branch_tracking)
        self.variable_tracking = kwargs.get("variable_tracking", settings.emulation.variable_tracking)
        self.memory_settings = kwargs.get("memory", {})

        self.disabled_rep = False
        self.teleported = False
        self.monitors = []
        if self.variable_tracking:
            self.add_monitor(VariableMonitor())

        self._instruction_hooks = InstructionHooks()
        self.add_monitor(self._instruction_hooks)

    def clear_cache(self):
        """
        Clears any internal caching during previous emulation runs.
        Calling this will be necessary if you have patched in new bytes into the disassembler and would like
        the emulator's memory object to reflect this change.
        """
        self._flowchart_cache.clear()
        self._memory_cache.clear()
        self.clear_tracing_cache()

    def enable(self, *names: str):
        """
        Enables the use of a specific opcode or function hook.

        The hooks enabled are pulled from the default implementation
            of opcodes/functions and will overwrite any custom hook currently in place.

        :param names: Name(s) of opcode/function hook.
            NOTE: All the "rep*" opcodes will be enabled if the name is "rep".
        """
        self.clear_cache()
        for name in names:
            name = name.lower()

            if name in self._context_class.OPCODES:
                self._opcode_hooks[name] = self._context_class.OPCODES[name]
            elif name in call_hooks.BUILTINS:
                self._call_hooks[name] = call_hooks.BUILTINS[name]
            elif name.startswith("rep"):
                self.disabled_rep = False
            else:
                raise ValueError(f'Opcode/function hook named "{name}" not found.')

    @property
    def enabled_opcodes(self) -> list[str]:
        """
        List of enabled opcode hooks.
        """
        return list(self._opcode_hooks.keys())

    @property
    def enabled_functions(self) -> list[str|int]:
        """
        List of enabled function call hooks.
        """
        return list(self._call_hooks.keys())

    def disable(self, *names: str):
        """
        Disables the use of a specific opcode or function hook.
        This is useful when trying to optimize processing time.

        WARNING: Disabling some opcodes like "call", "push", or "pop" may cause the stack pointer
            to be miss-aligned for future instructions. Do this at your own risk.

        :param names: Name(s) of opcode/function hook.
            NOTE: All the "rep*" opcodes will be enabled if the name is "rep".
        """
        self.clear_cache()
        for name in names:
            name = name.lower()
            if name in self._opcode_hooks:
                del self._opcode_hooks[name]
            elif name in self._call_hooks:
                del self._call_hooks[name]
            elif name.startswith("rep"):
                self.disabled_rep = True

    def disable_all(self, disable_function_hooks: bool = False):
        """
        Disables all opcode hooks for the current emulator instance.
        This is meant to be used when only a small number of opcodes need to
            be emulated and can greatly help speed up emulation.

        This removes all opcode hooks currently in place, only function hooks
            will be emulated.
        Function hooks can be disabled as well if desired.

        Enabling specific hooks can be done with :func:`~emulator.Emulator.enable`.

        The simplest way to have all the disabled hooks enabled again is to
            create a new emulator instance.
        """
        self.clear_cache()
        self._opcode_hooks = {}
        self._instruction_hooks.clear()
        self.disabled_rep = True
        logger.debug("All opcode/instruction hooks disabled")

        if disable_function_hooks:
            self._call_hooks = {}
            logger.debug("All function hooks disabled")

    def new_context(self) -> ProcessorContext:
        return self._context_class(self, self.memory_settings)

    def hook_call(self, name_or_start_ea, func, *, num_args: Union[Callable, int] = None):
        """
        Hooks all calls to a function with a custom user defined function.
        Useful for emulating the effects of a function or reporting data.

        :param name_or_start_ea: Name or starting address of the function to hook (e.g. 'base64')
        :param func: Function to run while emulating a call to that function.
            Function must accept 3 arguments: cpu_context, func_name, and func_args and
            may return a value to be set to the rax register (or equivalent)
        :param num_args: Forces a specific number of arguments to be passed to the function.
            This can be an integer or a callable that takes the function name or nothing as an argument.
        :return:
        """
        self.clear_cache()
        if isinstance(name_or_start_ea, str):
            name_or_start_ea = name_or_start_ea.lower()

        if num_args is not None:
            # Create a copy so we can manipulate the num_args without affecting any attributes on the original.
            # e.g. if a builtin_func is being used.
            func_original = func
            def func_copy(*args, **kwargs):
                return func_original(*args, **kwargs)
            func = func_copy
            func.num_args = num_args

        self._call_hooks[name_or_start_ea] = func

    def hook_instruction(self, opcode_or_ea, func, pre=True):
        """
        Hooks all instructions of a given opcode or at specific address with a custom
        user defined function.

        :param opcode_or_ea: name of the opcode or address of the instruction to hook (e.g. "pop")
        :param func: Function to run while before or after emulating the instruction.
            Function must accept 2 arguments: cpu_context, instruction
        :param pre: Whether to run the function before or after the instruction has been emulated.
            (defaults to before)
        """
        self.clear_cache()
        self._instruction_hooks.hook(opcode_or_ea, func, pre=pre)

    def hook_opcode(self, opcode, func):
        """
        Sets the callback function which implements the emulation for the given opcode.

        WARNING: Only one hook can be implemented for each opcode. If you set a hook, this will
        replace any existing implementation for the life of this Emulator instance.
        You probably want to be using `hook_instruction()` instead.

        NOTE: If you are hooking an opcode to overwrite a buggy implementation, please
        consider contributing a bugfix. :)

        :param opcode: name of the opcode to implement. (e.g. "pop")
        :param func: Function to run to emulate the opcode.
            Function must accept 2 arguments: cpu_context, instruction
        """
        self.clear_cache()
        self._opcode_hooks[opcode.lower()] = func

    def emulate_call(self, name_or_start_ea, call_depth: int = 0):
        """
        Defines whether a call to a specific function should be emulated using a combination
        of create_emulated() and hook_call()

        :param name_or_start_ea: Name or starting address of the function to emulate all calls to
        :param call_depth: Number of function calls we are allowed to emulate into.
            When we hit our limit (depth is 0), emulation will no longer jump into function calls.
            (Defaults to not emulating into any function calls.)
            NOTE: This does not affect call hooks.
        """

        if isinstance(name_or_start_ea, str):
            name = name_or_start_ea
            # NOTE: Using from_name because we need to be sure there is actual instructions to emulate.
            func = self.disassembler.get_function_by_name(name)
            func_address = func.start
        else:
            func_address = name_or_start_ea

        def hook(context, func_name, func_args):
            self.execute_function(func_address, call_depth=call_depth, context=context)

        self.hook_call(name_or_start_ea, hook)

    def clear_hooks(self):
        """Clears all currently set hooks (including builtin ones)."""
        self.reset_hooks()
        self._call_hooks = {}

    def reset_hooks(self):
        """Resets hooks back to the default builtin ones."""
        self.clear_cache()
        self._call_hooks = call_hooks.BUILTINS.copy()
        self._instruction_hooks.clear()
        self._opcode_hooks = self._context_class.OPCODES.copy()

    def get_call_hook(self, func_name_or_start_ea):
        """
        Gets function call hook for given function name or start address.

        :param func_name_or_start_ea: Name or start address of the function to get hook for.

        :return: A function to run or None if there is no hook for the given function.
        :rtype: function
        """
        # Convert the string to lowercase so it can match the dictionary of built-in functions
        if isinstance(func_name_or_start_ea, str):
            func_name_or_start_ea = func_name_or_start_ea.lower()

        return self._call_hooks.get(func_name_or_start_ea)

    def get_instruction_hooks(self, opcode_or_ea, pre=True):
        """
        Gets instruction hook for given opcode mnemonic or address.

        :param opcode_or_ea: Opcode mnemonic or address of the instruction
        :param pre: Whether to run the function before or after the instruction has been emulated.
            (defaults to before)

        :return: A list of hook functions.
        """
        return self._instruction_hooks.get(opcode_or_ea, pre=pre)

    def get_opcode_hook(self, opcode) -> Optional[callable]:
        """
        Gets the opcode implementation for the given opcode mnemonic.
        :param opcode: Name of the opcode to get opcode from.
        :return: The function callback or None if there is not hook.
        """
        return self._opcode_hooks.get(opcode.lower())

    def add_monitor(self, monitor: Monitor = None, /, **kwargs: Unpack[CallbacksMonitor.Args]):
        """Adds an emulation monitor."""
        if not monitor:
            if not kwargs:
                raise ValueError(f"Expect monitor or callback functions to be provided.")
            monitor = CallbacksMonitor(**kwargs)
        self.monitors.append(monitor)

    def remove_monitor(self, monitor: Monitor):
        try:
            self.monitors.remove(monitor)
        except ValueError:
            pass

    @contextlib.contextmanager
    def monitor(self, monitor: Monitor = None, /, **kwargs: Unpack[CallbacksMonitor.Args]) -> Monitor:
        """
        Temporarily enables a monitor to the emulator.
        """
        if not monitor:
            if not kwargs:
                raise ValueError(f"Expect monitor or callback functions to be provided.")
            monitor = CallbacksMonitor(**kwargs)

        self.add_monitor(monitor)
        try:
            yield monitor
        finally:
            self.remove_monitor(monitor)

    @contextlib.contextmanager
    def execution_limit(self, max_instructions: int = None):
        """
        Limits emulation to a maximum number of instructions.
        """
        if max_instructions is None:
            max_instructions = self.max_instructions
        with self.monitor(MaxExecutionMonitor(max_instructions)):
            yield

    def execute(self, start: int, end: int = None, call_depth: int = 0, *, context: ProcessorContext = None) -> ProcessorContext:
        """
        Emulates from start instruction to end instruction (not including the end instruction)
        (Or just emulates the start instruction if end is not provided.)

        Emulation will stop when either the end instruction is reached or the max number of instructions
        have been emulated. Whichever comes first.

        Loops will be emulated as expected. If you would like to force emulation down the path to the
        end instruction, use iter_context_at() or context_at() instead.

        :param start: Address of instruction to start emulation.
        :param end: Address of instruction to stop emulation. (non-inclusive)
        :param call_depth: Number of function calls we are allowed to emulate into.
            When we hit our limit (depth is 0), emulation will no longer jump into function calls.
            (Defaults to not emulating into any function calls.)
            NOTE: This does not affect call hooks.
        :param context: A premade context that you would like to use to start out emulation.
            In which case, this is just a wrapper for running execute() on the context itself.

        :returns: A ProcessorContext object after emulation has occurred.
            If a context was provided in the parameters, this context will be a reference to that.
        """
        if not context:
            context = self.new_context()
        context.execute(start=start, end=end, call_depth=call_depth)
        return context

    def execute_function(self, address_or_name: Union[int, str], call_depth: int = 0, *, context: ProcessorContext = None):
        """
        Emulates the full function.

        :param address_or_name: An address contained within the function or name of function.
        :param call_depth: Number of function calls we are allowed to emulate into.
            When we hit our limit (depth is 0), emulation will no longer jump into function calls.
            (Defaults to not emulating into any function calls.)
            NOTE: This does not affect call hooks.
        :param context: A premade context that you would like to use to start out emulation.

        :returns: A ProcessorContext object after emulation has occurred.
            If a context was provided in the parameters, this context will be a reference to that.

        :raises NotExistError: If a function doesn't exist at the given address.
        """
        if isinstance(address_or_name, str):
            name = address_or_name
            func = self.disassembler.get_function_by_name(name)
        else:
            address = address_or_name
            func = self.disassembler.get_function(address)
        return self.execute(start=func.start, end=func.end, call_depth=call_depth, context=context)

    def _execute_to(self, address, *, call_depth: int = 0, context: ProcessorContext = None) -> ProcessorContext:
        """
        Creates a cpu_context (or emulates on top of the given one) for instructions up to, but not
        including, the given ea within the current function.
        This function is a hybrid approach to the non-loop following mode in which it will
        force the other branch to be taken if the branch it wants to take will not lead to the
        desired end address.

        This is an internal function used as a helper for iter_context_at() when following loops.

        :param int address: Address of interest
        :param call_depth: Number of function calls we are allowed to emulate into.
            When we hit our limit (depth is 0), emulation will no longer jump into function calls.
            (Defaults to not emulating into any function calls.)
            NOTE: This does not affect call hooks.
        :param context: ProcessorContext to use during emulation, a new one will be created if not provided.

        :raises RuntimeError: If maximum number of instructions have been hit.
        """
        if not context:
            context = self.new_context()

        function = self.disassembler.get_function(address)
        flowchart = function.flowchart

        start_block = flowchart.get_block(function.start)
        end_block = flowchart.get_block(address)
        valid_blocks = set(end_block.ancestors)
        valid_blocks.add(end_block)

        # Starting from start_block, we are going to emulate each instruction in each basic block
        # until we get to the end_block.
        # If execution tries to branch us into a block that can't lead us to the end_block,
        # we will force the branch to go in the other direction.
        with self.execution_limit(self.max_instructions):
            current_block = start_block
            while current_block != end_block:
                context.execute_block(current_block, call_depth=call_depth)

                # Get the successor block that execution branched to as well as
                # a valid block that can reach the end block.
                # If no such block exists, just pick the first valid successor block.
                valid_successors = [bb for bb in current_block.blocks_from() if bb in valid_blocks]
                assert valid_successors, "Expected there to be at least 1 valid successor block."
                for successor in valid_successors:
                    if context.ip == successor.start:
                        break
                else:
                    # If no valid successor, force branch.
                    successor = valid_successors[0]
                    context.ip = successor.start

                current_block = successor

        # Emulate the instructions in the final block.
        context.execute_block(current_block, end=address, call_depth=call_depth)

        return context

    def _init_contexts(self, address: int, depth, call_depth, exhaustive, follow_loops, init_context):
        """
        Helper function to generate initial context for the beginning of a function.
        """
        logger.debug("Iterating contexts for call level: %d", depth)

        if not depth:
            yield init_context
            return

        func = self.disassembler.get_function(address)
        yielded = False
        for call_ea in func.calls_to():
            if call_ea in func:
                logger.warning("Ignoring recursive function call at 0x%08X", call_ea)
                continue
            try:
                self.disassembler.get_function(call_ea)
            except dragodis.NotExistError:
                logger.warning("Ignoring call at 0x%08X. Not in a function", call_ea)
                continue
            for context in self.iter_context_at(
                call_ea,
                depth=depth - 1,
                call_depth=call_depth,
                exhaustive=exhaustive,
                follow_loops=follow_loops,
                init_context=init_context,
                _first_call=False
            ):
                if issubclass(self._context_class, x86_64ProcessorContext):
                    # increase the sp to account for the return address that gets pushed
                    # onto the stack so that we are aligned correctly.
                    context.sp -= context.byteness

                # yield a context containing the caller executed first.
                yield context
                yielded = True
        # If we didn't yield, then we hit a function that has no callers or valid contexts.
        if not yielded:
            yield init_context

    def iter_exhaust(
            self,
            start: int = None, *,
            depth: int = 0,
            call_depth: int = 0,
            exhaustive: bool = False,
            follow_loops: bool = False,
            scope: str = "instruction",
            ignore_libraries=True,
            pre: bool = True,
            init_context: ProcessorContext = None,
    ) -> Iterable[tuple[ProcessorContext, Instruction]]:
        """
        Iteratively executes all code paths for given function or all functions.

        :param start: Address of instruction to start emulating.
            If provided, only the function that contains the given instruction will be emulated.
        :param depth: Number of calls up the stack to pull context from.
            (defaults to 0, meaning emulation will start at the start of the current function)
        :param call_depth: Number of function calls we are allowed to emulate into.
        :param exhaustive:
            If true, all paths for each call level depth is processed.
            If follow_loops is also true, this will ensure loops are followed at each call level depth.
            If false, only the first path for each depth is processed.
            If follow_loops is also true, loops will only be followed for the first call level.
                All other levels will use the non-follow_loops method.
        :param follow_loops:
            If true, loops will be followed during emulation and only one possible
            path will be emulated per call level. ("function" scope will only end at the true end of the function)
            If false, emulation will be forced down a specific path of flowchart blocks in order
            to get to the given address. ("function" scope can end when a loop is encountered)
        :param ignore_libraries: Whether to ignore detected libraries when emulating all functions.
        :param scope: Determines when to yield context and current instruction:
            "instruction": yields before or after every instruction is executed. (default)
            "block": yields at the end of a block.
            "code_path": yields at the end of the code path.
            "function': yields at the end of a function.
        :param pre: Whether to yield before or after the current instruction is emulated. (instruction scope)
        :param init_context: Initial context to use during emulation, a new one will be created if not provided.
        """
        if scope not in ("instruction", "block", "code_path", "function", "none"):
            # NOTE: "none" is internally used for `exhaust()`
            raise ValueError(f"Invalid scope '{scope}'. Must be one of: 'instruction', 'block', 'code_path', 'function'")

        # If start address not provided, iterate from the start of all functions.
        if start is None:
            for func in self.disassembler.functions():
                if ignore_libraries and func.is_library:
                    continue
                yield from self.iter_exhaust(
                    func.start, depth=depth, call_depth=call_depth, exhaustive=exhaustive, follow_loops=follow_loops,
                    scope=scope, pre=pre, init_context=init_context
                )
            return

        if init_context:
            context = deepcopy(init_context)
        else:
            context = self.new_context()

        # If depth provided, iterate from initial contexts built from callers to the current function.
        if depth:
            for context in self._init_contexts(
                    start, depth=depth, call_depth=call_depth, exhaustive=exhaustive, follow_loops=follow_loops,
                    init_context=context
            ):
                # Don't include depth, already handled with _init_contexts()
                yield from self.iter_exhaust(
                    start, call_depth=call_depth, exhaustive=exhaustive, follow_loops=follow_loops,
                    scope=scope, pre=pre, init_context=context
                )
            return

        flowchart = self.disassembler.get_flowchart(start)
        start_block = flowchart.get_block(start)
        visited = set()

        with self.execution_limit():
            states: list[tuple[BasicBlock, ProcessorContext, int]] = [(start_block, context, start)]
            while states:
                block, context, start = states.pop()
                visited.add(block)

                logger.debug("Emulating instructions 0x%08X -> 0x%08X", start, block.end)

                if scope == "instruction":
                    for instruction in context.iter_execute_block(start, call_depth=call_depth, pre=pre):
                        yield context, instruction
                else:
                    context.execute_block(block, call_depth=call_depth)

                if scope == "block":
                    yield context, context.prev_instruction

                # Add states for the next code blocks.
                terminal = True
                blocks_from = sorted(block.blocks_from(), reverse=True)
                for child_block in blocks_from:
                    if follow_loops or child_block not in visited:
                        states.append((child_block, deepcopy(context), child_block.start))
                        terminal = False

                if terminal:
                    # We are at the end of a code path, run hooks.
                    for monitor in self.monitors:
                        monitor.code_path_end(context, context.prev_instruction)
                    if scope == "code_path" or (scope == "function" and not blocks_from):
                        yield context, context.prev_instruction

    def exhaust(self, *args, **kwargs: Unpack[IterExhaustArgs]):
        """
        Executes all code paths for given function or all functions.
        This doesn't return anything. Instead, this is intended to be used with a monitor to collect results.
        """
        # deque is a performant way to consume an iterator.
        collections.deque(self.iter_exhaust(*args, scope="none", **kwargs), maxlen=0)

    def iter_context_at(
            self, address: int, *, depth=0, call_depth: int = 0, exhaustive=True, follow_loops=False, init_context=None, _first_call=True
    ) -> Iterable[ProcessorContext]:
        """
        Iterate over cpu context for instructions up to, but not including, a given ea.
        (within the current function)

        >>> emu = Emulator()
        >>> for cpu_context in emu.iter_context_at(0x1001b9ad):
        >>>     print(cpu_context)

        :param int address: address of interest
        :param int depth: Number of calls up the stack to pull context from.
            (defaults to 0, meaning emulation will start at the start of the function containing the given address)
        :param call_depth: Number of function calls we are allowed to emulate into.
            When we hit our limit (depth is 0), emulation will no longer jump into function calls.
            (Defaults to not emulating into any function calls.)
            NOTE: This does not affect call hooks.
        :param bool exhaustive:
            If true, all paths for each call level depth is processed.
            If follow_loops is also true, this will ensure loops are followed at each call level depth.
            If false, only the first path for each depth is processed.
            If follow_loops is also true, loops will only be followed for the first call level.
                All other levels will use the non-follow_loops method.
        :param follow_loops:
            If true, loops will be followed during emulation and only one possible
            path will be emulated per call level.
            If false, emulation will be forced down a specific path of flowchart blocks in order
            to get to the given address.
        :param init_context: Initial context to use to start emulation.
            NOTE: The yielded context will be a copy of the passed in context with emulation applied.
        :param _first_call: Detects if we are the first call in a recursive call. Please don't change this.

        :yield: cpu_context
        """
        if depth < 0:
            raise ValueError("Depth must be a positive integer.")

        if init_context and init_context.emulator != self:
            raise ValueError("Emulator of supplied init_context must be the same.")

        if not init_context:
            init_context = self.new_context()
        else:
            # Create a copy of the context in order to follow the path of least surprises.
            init_context = deepcopy(init_context)

        logger.debug(
            f"Emulating call level %d for function at 0x%08X: follow_loops = %r, exhaustive = %r",
            depth, address, follow_loops, exhaustive
        )

        # Iterate contexts at this level.

        if follow_loops and (_first_call or exhaustive):
            for context in self._init_contexts(address, depth, call_depth, exhaustive, follow_loops, init_context):
                yield self._execute_to(address, call_depth=call_depth, context=deepcopy(context))

        else:
            for context in self._init_contexts(address, depth, call_depth, exhaustive, follow_loops, init_context):
                flowchart = self.disassembler.get_flowchart(address)
                for path in iter_paths(flowchart, address, _cache=self._flowchart_cache):
                    yield path.cpu_context(address, call_depth=call_depth, init_context=deepcopy(context))

                    # Don't process other paths if we are at the user call level and exhaustive wasn't chosen.
                    if not _first_call and not exhaustive:
                        break

    def context_at(self, address: int, /, **kwargs: Unpack[IterContextArgs]) -> ProcessorContext:
        """
        Obtain a cpu context for instructions up to, but not including, a given ea.

        :param int address: address of interest
        :param kwargs: iter_context_at() arguments

        :return: cpu_context
        :raise ValueError: If no code paths were discovered at the given address. (Usually means address is not code.)
        """
        for ctx in self.iter_context_at(address, **kwargs):
            return ctx
        raise ValueError(f"Failed to discover any code paths to address 0x{address:08x}")

    def iter_operand_value(self, address: int, index: int, **kwargs: Unpack[IterContextArgs]) -> Iterable[Tuple[ProcessorContext, int]]:
        """
        Trace the function to the specified ea and yield all possible values for the operand.
        This is a helper wrapper for extracting the context and then retrieving either
        the memory address or raw value from the operand.

        >>> emu = Emulator()
        >>> for ctx, val in emu.iter_operand_value(0x1001b9ad, 0):
        >>>     print(f"Val for opnd0 at 0x1001b9ad = 0x{val:x}")

        :param int address: address to trace to
        :param int index: the operand of interest (0 - first operand, 1 - second operand, ...)

        :yield tuple: (context at ea, operand value)
        """
        values = set()

        # Iterate all the nodes to obtain the CPU context
        for cpu_context in self.iter_context_at(address, **kwargs):
            operand = cpu_context.operands[index]
            # Pass memory address if there is one, otherwise pass the value.
            value = operand.addr or operand.value
            # Prevent returning multiple values which are the same....
            if value in values:
                continue

            values.add(value)
            yield cpu_context, value

    def get_operand_value(self, address: int, index: int, **kwargs: Unpack[IterContextArgs]) -> Optional[Tuple[ProcessorContext, int]]:
        """
        Trace the function to the specified ea and return the value for the specified operand.
        This is a helper wrapper for extracting the context and then retrieving either
        the memory address or raw value from the operand.

        NOTE: We are using "get_operand_value" to help show this is the equivalent to
        idc.get_operand_value() but pulls from emulated data.

        :param int address: address to trace to
        :param int index: the operand of interest (0 - first operand, 1 - second operand, ...)

        :returns tuple: (context at ea, operand value)
        """
        for cpu_context, value in self.iter_operand_value(address, index, **kwargs):
            return cpu_context, value

    def iter_function_args(
            self, address: int, *, num_args=None, **kwargs: Unpack[IterContextArgs]
    ) -> Iterable[Tuple[ProcessorContext, List[FunctionArgument]]]:
        """
        Given the EA of a function call, attempt to determine the number of arguments passed to the function and
        return those values to the caller.  Additionally, give back the context as well since it may be useful.

        >>> emu = Emulator()
        >>> call_addr = 0x1001b9d0
        >>> for context, args in emu.iter_function_args(call_addr):
        >>>     print("Args for call at 0x{:X}: {}".format(call_addr, ", ".join(args)))

        :param int address: address containing the function call of interest
        :param int num_args: Force a specific number of arguments.
            If not provided, number of arguments is determined by the disassembler.
            Extra arguments not defined by the disassembler are assumed to be 'int' type.
        :param kwargs: iter_context_at() arguments.

        :yield tuple: (context at ea, list of function parameters passed to called function in order)
        """
        # Iterate all the paths leading up to ea
        for cpu_context in self.iter_context_at(address, **kwargs):
            yield cpu_context, cpu_context.get_function_args(num_args=num_args)

    def get_function_args(
            self, address: int, *, num_args=None, **kwargs: Unpack[IterContextArgs]
    ) -> Optional[Tuple[ProcessorContext, List[FunctionArgument]]]:
        """
        Simply calls iter_function_args with the provided ea and returns the first set of arguments.

        :return tuple: (context at ea, list of function arguments passed to called function in order)
        """
        for cpu_context, args in self.iter_function_args(address, num_args=num_args, **kwargs):
            return cpu_context, args

    def get_function_arg_values(
            self, address: int, *, num_args=None, **kwargs: Unpack[IterContextArgs]
    ) -> Optional[Tuple[ProcessorContext, List[int]]]:
        """
        Simply calls iter_function_args with the provided ea and returns the first set of argument values.

        :return tuple: (context at ea, list of function argument values passed to called function in order)
        """
        for cpu_context, args in self.iter_function_args(address, num_args=num_args, **kwargs):
            return cpu_context, [arg.value for arg in args]

    def create_emulated(self, func_ea, return_type=None, return_size=None, enforce_args=False) -> Callable:
        """
        Creates a Python function that emulates the execution of the given function.

        :param func_ea: Address of function to emulate.
        :param return_type: If set, return value will be dereferenced using set type before returning.
            (e.g. function_tracing.DWORD)
        :param return_size: If set, return value will be dereferenced as a bytes string of the given
            size before returning.
        :param enforce_args: Whether to enforce that the correct number of positional arguments
            was provided.

        :returns: A python function that emulates the function.
            This function will accept the same number (and type) of arguments as the
            emulate function as well as return the result as the emulated function.

        :raises RuntimeError: If the maximum number of allowed instruction executions have been reached.
        """
        func_obj = self.disassembler.get_function(func_ea)

        def emulated_function(*args, context: ProcessorContext = None):
            """
            Emulates a function and returns result in rax.

            :param *args: Arguments to pass into function before emulation.
                If enforce_args is not enabled, the number of arguments provided can be less or more
                than the number of arguments required by the function.
                Any arguments not provided will default to whatever is set in the context.
            :param context: CPU context to use. If not provided an empty context will be used.
                If you would like to examine the context after emulation, you must provide your own.

            :returns: Value or derefernced value in rax.

            :raises TypeError: If enforce_args is enabled and incorrect number of positional args have been provided.
            """
            if context and context.emulator != self:
                raise ValueError("Supplied context must be created from same emulator.")
            context = context or self.new_context()
            # (ip must be set in order to get correct function arguments in signature.)
            context.ip = func_obj.start

            # Temporarily turn off branch tracking since it is unneeded and will just waste time.
            orig_branch_tracking = self.branch_tracking
            self.branch_tracking = False

            # Fill in context with argument values.
            func_sig = context.get_function_signature(func_obj.start)
            if enforce_args and len(func_sig.arguments) != len(args):
                raise TypeError(
                    f"Function takes {len(func_sig.arguments)} positional arguments, but {len(args)} were given.")

            logger.debug(f'Emulating {func_sig.name}')
            for arg, func_arg in zip(args, func_sig.arguments):
                if isinstance(arg, int):
                    logger.debug(f'Setting argument {func_arg.name} = {repr(arg)}')
                    func_arg.value = arg
                elif isinstance(arg, bytes):
                    ptr = context.memory.alloc(len(arg))
                    context.memory.write(ptr, arg)
                    logger.debug(f'Setting argument {func_arg.name} = {hex(ptr)} ({repr(arg)})')
                    func_arg.value = ptr
                else:
                    raise TypeError(f'Invalid arg type {type(arg)}')

            context.execute(func_obj.start, end=func_obj.end)

            if return_type is not None or return_size is not None:
                result = context.memory.read_data(context.ret, size=return_size, data_type=return_type)
            else:
                result = context.ret

            logger.debug(f'Returned: {repr(result)}')
            self.branch_tracking = orig_branch_tracking
            return result

        return emulated_function
