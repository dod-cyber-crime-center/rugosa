"""
Functions found in kernel32.dll
...
def get_environment_variable(cpu_context, func_name, func_args):
    ...
    var_name = cpu_context.memory.read_string(var_name_ptr, wide=wide)

    # Replace some common environment variables, and add %'s for others.
    # - 1 for null terminator
    name = settings.env.get(var_name.lower(), f"%{var_name}%")[:max_size - 1]
    cpu_context.memory.write_string(buffer_ptr, name, wide=wide)
...
def get_current_directory(cpu_context, func_name, func_args):
    ...
    cwd = settings.machine.working_directory[:max_size]

    logger.debug("Writing current working directory %s to 0x%08X", cwd, buffer_ptr)
    cpu_context.memory.write_string(buffer_ptr, cwd, wide=wide)
...
def get_module_file_name(cpu_context, func_name, func_args):
    ...
    # Since we shouldn't expose the user's real file path structure we'll use user defined setting instead.
    # Must be truncated to fit in max_size filename_ptr (-1 for the terminator)
    disassembler = cpu_context.emulator.disassembler
    file_path = f"{settings.machine.input_file_directory}\\{disassembler.input_path.name}"
    file_path = file_path[:max_size - 1]

    logger.debug("Writing module path %s to 0x%08X", file_path, filename_ptr)
    cpu_context.memory.write_string(filename_ptr, file_path, wide=wide)
...
def get_short_path_name(cpu_context, func_name, func_args):
    ...
    long_path = cpu_context.memory.read_string(long_path_ptr, wide=wide)

    short_path = long_path[:short_path_size]
    cpu_context.memory.write_string(short_path_ptr, short_path, wide=wide)
...
def get_system_directory(cpu_context, func_name, func_args):
    ...
    system_dir = settings.machine.system_directory[:max_size]

    logger.debug("Writing system directory %s to 0x%08X", system_dir, buffer_ptr)
    cpu_context.memory.write_string(buffer_ptr, system_dir, wide=wide)
...
def get_temp_path(cpu_context, func_name, func_args):
    ...
    temp_dir = settings.machine.temp_directory
    if not temp_dir.endswith("\\"):
        temp_dir += "\\"
    temp_dir = temp_dir[:max_size]

    logger.debug("Writing temp directory %s to 0x%08X}", temp_dir, buffer_ptr)
    cpu_context.memory.write_string(buffer_ptr, temp_dir, wide=wide)
...
def get_tick_count(cpu_context, func_name, func_args):
    ...
    """
    tick_count = settings.machine.tick_count
    if tick_count == "now":
        mask = 0xFFFFFFFFFFFFFFFF if func_name == "GetTickCount64" else 0xFFFFFFFF
        tick_count = int(time.time()) & mask
    return tick_count
...
def get_windows_directory(cpu_context, func_name, func_args):
    ...
    win_dir = settings.machine.windows_directory[:max_size]
    logger.debug("Writing windows directory %s to 0x%08X", win_dir, buffer_ptr)
    cpu_context.memory.write_string(buffer_ptr, win_dir, wide=wide)
...
def get_computer_name(cpu_context, func_name, func_args):
    ...
    computer_name = settings.machine.computer_name
...
def get_user_name(cpu_context, func_name, func_args):
    ...
    user_name = settings.machine.username
...
def create_process(cpu_context, func_name, func_args):
    ...
    cmd = cpu_context.memory.read_string(cmd_ptr, wide=wide)
    if app_ptr:
        app = cpu_context.memory.read_string(app_ptr, wide=wide)
        cmd = app + " " + cmd
...
def win_exec(cpu_context, func_name, func_args):
    ...
    cmd = cpu_context.memory.read_string(cmd_ptr)
...
def create_directory(cpu_context, func_name, func_args):
    ...
    path = cpu_context.memory.read_string(path_ptr, wide=wide)
...
def create_file(cpu_context, func_name, func_args):
    ...
    path = cpu_context.memory.read_string(name_ptr, wide=wide)
...
def write_file(cpu_context, func_name, func_args):
    ...
    data = cpu_context.memory.read(data_ptr, num_bytes_to_write)
    cpu_context.actions.add(actions.FileWritten(cpu_context.ip, handle, data))

    if bytes_written_ptr:
        cpu_context.memory.write_data(bytes_written_ptr, len(data), data_type=constants.DWORD)
