...
def strcat(cpu_context, func_name, func_args):
    ...
    if dst and src:
        ...
        append_str = cpu_context.memory.read_string_bytes(src, wide=wide)
        dest_str = cpu_context.memory.read_string_bytes(dst, wide=wide)
...
def strncat(cpu_context, func_name, func_args):
    ...
    if dst and src:
        logger.debug("Concatenating c string in 0x%X to c string in 0x%X", src, dst)
        append_str = cpu_context.memory.read_string_bytes(src, wide=wide)[:count]
        dest_str = cpu_context.memory.read_string_bytes(dst, wide=wide)
        null_offset = dst + len(dest_str)
        if dst_size is not None:
            dst_size -= null_offset - dst
...
def strcpy(cpu_context, func_name, func_args):
    ...
    if dst and src:
        ...
        terminator = b"\0\0" if wide else b"\0"
        src_str = cpu_context.memory.read_string_bytes(src, wide=wide) + terminator
        size = len(src_str)
        if dst_size is not None:
...
def strlen(cpu_context, func_name, func_args):
    ...
    if str_ptr:
        ...
        sstr = cpu_context.memory.read_string_bytes(str_ptr, wide=wide)
        size = len(sstr)
        if wide:
            size /= 2
