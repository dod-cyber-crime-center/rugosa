...
def sanitize_func_name(func_name: str) -> str:
    ...
    # (Avoid stripping for Vivisect)
    func_name = re.sub("(?<!^sub)_[0-9]+$", "", func_name)
...
class _CachedGeneratorFunction(Protocol):
    """ A function / bound-method that returns a cached iterator. """
    def __call__(self, *args: Any, **kwargs: Any) -> Iterator[Any]: ...
    def clear_cache(self, _self: Any = None) -> None: ...


def cached_generator(func) -> _CachedGeneratorFunction:
    """
    Creates a generator with cached results such that if another generator is created
    with the same arguments a tee'd version of the previously created generator is returned.
    """
    _global_cache = {}
    attr = f"__cache_{func.__name__}"

    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        if args and hasattr(args[0], "__dict__"):   # per-instance cache
            cache = getattr(args[0], attr, {})
            setattr(args[0], attr, cache)
        else:
            cache = _global_cache
        ...
        cache[key], gen = itertools.tee(gen)
        return gen

    # add option to clear the cache.
    def clear_cache(self=None):
        cache = getattr(self, attr, {}) if self else _global_cache
        cache.clear()
