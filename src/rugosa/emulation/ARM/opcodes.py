...
def STM(cpu_context: ProcessorContext, instruction: Instruction):
    ...
    reg_values = operands[1].value
    # Operand should be a register list, but the disassembler
    # may display the operand as a single register if there is only one.
...
def PUSH(cpu_context: ProcessorContext, instruction: Instruction):
    ...
    # may display the operand as a single register if there is only one.
    if not isinstance(reg_values, list):
        logger.debug("Casting operand '%s' to a list --> [0x%X]", operands[0].text, reg_values)
        reg_values = [reg_values]
