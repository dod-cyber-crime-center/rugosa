...
import collections
import logging
from typing import List, Tuple, Optional, TYPE_CHECKING, Any, Union, Generator
...
if TYPE_CHECKING:
    from .emulator import Emulator, MemoryArgs
...
class JccContext:
    ...
    def __init__(self):
        ...
        self.flag_opnds: dict[str, list[OperandLite]] = {}  # Dictionary containing the operands at a particular instruction which set
        # specific flags.  Dictionary is keyed on flag registry names.
    ...
    def update_flag_opnds(self, flags: list[str], opnds: list[Operand]):
        """
    ...
    def get_flag_opnds(self, flags: list[str]) -> list[OperandLite]:
        """
    ...
    def is_alt_branch(self, ip: int) -> bool:
        """
...
class ProcessorContext:
    ...
    :param memory_settings: Settings for emulated memory (overwrites `memory` section in settings.toml)
    """
    ...
    def __init__(
            ...
            stack_pointer: str,
            return_register: str,
            memory_settings: MemoryArgs,
    ):
        self.emulator = emulator
        self.registers = registers
        self.jcccontext = JccContext()
        self.memory = Memory(self, **memory_settings)
    ...
    def disassembler(self):
        """Shortcut to disassembler."""
        return self.emulator.disassembler
    ...
    @property
    def function_start(self) -> int:
        """
        The address for the start of the function we are currently in (if we are in one).
        """
        func = self.emulator.disassembler.get_function(self.ip, None)
        return func and func.start
    ...
    def execute(self, start=None, end=None, call_depth: int = 0, max_instructions: int = None) -> bool:
        ...
        if end is not None:
            ...
            with self.emulator.execution_limit(max_instructions):
    ...
    def iter_execute_block(
            self, start_or_block: Union[int, BasicBlock] = None, *, end: int = None, call_depth: int = 0,
            pre: bool = True,
    ) -> Generator[Instruction]:
        """
        Iteratively executes a basic block for the given start address.

        :param start_or_block: address to start execution (defaults to currently set ip)
            Alternatively, this can be a BasicBlock object.
        :param end: address to stop execution assuming its within the block (not including)
            (defaults to execute up to the end of the block)
        :param call_depth: Number of function calls we are allowed to emulate into.
            When we hit our limit (depth is 0), emulation will no longer jump into function calls.
            (Defaults to not emulating into any function calls.)
            NOTE: This does not affect call hooks.
        :param pre: Whether to yield the instruction before or after the emulation.

        :yields: Instruction
        """
        if start_or_block is None:
            start_or_block = self.ip

        if isinstance(start_or_block, int):
            start = start_or_block
            block = self.disassembler.get_basic_block(start)
        else:
            block = start_or_block
            start = block.start

        # Modify the context for the current branch if required
        if start == block.start:
            self.prep_for_branch(start)

        self.ip = start
        for monitor in self.emulator.monitors:
            monitor.block_start(self, self.instruction)

        last_address = self.disassembler.get_line(block.end - 1).address
        for line in block.lines(start=start):
            if end is not None and line.address == end:
                break
            instruction = self.get_instruction(line.address)
            if pre:
                yield instruction
            self.execute(line.address, call_depth=call_depth)
            if line.address == last_address:
                for monitor in self.emulator.monitors:
                    monitor.block_end(self, instruction)
            if not pre:
                yield instruction

    def execute_block(self, start_or_block: Union[int, BasicBlock] = None, end: int = None, call_depth: int = 0):
        """
        Executes a basic block for the given start address.
        """
        # Consumes the iterator.
        collections.deque(self.iter_execute_block(start_or_block, end=end, call_depth=call_depth), maxlen=0)
    ...
    def _execute_call(self, func_address: int, func_name: str = None, call_address: int = None):
        ...
        try:
            ...
            num_args = None
            if call_hook and hasattr(call_hook, "num_args"):
                num_args = call_hook.num_args
                if callable(num_args):
                    try:
                        num_args = num_args(func_name)
                    except TypeError:
                        num_args = num_args()
